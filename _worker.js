...
export default {
    async fetch(request, ctx) {
      try {
        ...
        // Retrieve `listProxy` from Cloudflare KV
        const listProxyJSON = await PROXY_LIST.get('78ea13b24a75421d912a35945abb9904');
        const listProxy = JSON.parse(listProxyJSON);

        // Find the proxy for the requested path
...
async function getAllConfigVless(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const clashConfTls = 
`- name: ${data.isp} (${data.countryCode}) ${proxy}
  server: ${hostName}
  port: 443
  type: vless
  uuid: ${generateUUIDv4()}
...
<div class="config-section" style="background-color: rgba(10, 10, 10, 0.8); color: #00ff00; border: 2px solid #00ff00;">
    ...
    <p style="color: #00ff00;"><strong>IP Proxy:</strong> ${proxy}</p>
    ...
    <div class="container">
        <div class="header">
            <div class="profile-container">
				<h2 class="profile-name">DANY PRATMANTO</h2>
			</div>
        </div>
